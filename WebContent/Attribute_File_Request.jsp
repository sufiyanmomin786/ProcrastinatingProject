	...
	<section class="container">
		...
		<div class="row">
            ...
            <TR class="danger">            	
               <TH>User Name</TH>
               <TH>File Name</TH>
               <Th>Send Key and Attribute</Th>
               <Th>Reject Request</Th>
               <Th>Duration_Data</Th>
            </TR>
<% 
	String query="select * from request where Status='Pending' and RequestName='KeyandAttribute'";
	Connection con=ConnectionFactory.getInstance().getConnection();
    Statement st=con.createStatement();
	ResultSet resultset=st.executeQuery(query);
	while(resultset.next())
	{
		String Duration_Data=resultset.getString("Duration_Data");
		if(!Duration_Data.equals("0"))
		{
%>
 <TR>
               <TD> <%= resultset.getString(2) %></TD>
               <TD> <%= resultset.getString(3) %></TD>
              <TD> <a href="Key_AttributeResponse?filename=<%=resultset.getString(3)%>&user=<%=resultset.getString(2)%>&ownername=<%=resultset.getString(4)%>"><input type="button" name="sendkey" value="Send Key and Attribute" class="btn btn-primary"></a></TD>
              <TD> <a href="Attribute_File_Reject?filename=<%=resultset.getString(3)%>&user=<%=resultset.getString(2)%>&ownername=<%=resultset.getString(4)%>"><input type="button" name="sendkey" value="Reject" class="btn btn-primary"></a></TD>
